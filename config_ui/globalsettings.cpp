...
void SaveableToStorage::switchSubgroup(int id)
{
    ...
    // when it switched it tries to read value, if absent - uses current state as default which if
    // absent defaults to default i.e. implicit copy of the prev state used (prev. subgroup)
...
SaveableToStorage::SaveableToStorage(const QString &key, const QVariant &def,
                                     const QString &group) :
    key_m(key),
    default_m(def),
    currSubgroup(0),
...
QList<QWidget *> StaticSettingsMap::createWidgets() const
{
    ...
    for (const auto &p : sets)
    {
        ...
        r.back()->setProperty("SettKey", p.first); // this is settings name/key bound with this
                                                   // widget, just in case we want it ever later
    }
...
// allowing copy-paste from header (using same name for macro), needed to have 1 virtual table for
// all objects, otherwise each inst has own = bigger code
#define DECL_DESTRUCTOR(NAME)                                                                      \
    NAME::~NAME()                                                                                  \
    {                                                                                              \
    }
