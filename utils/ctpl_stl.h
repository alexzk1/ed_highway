...
class thread_pool
{
  ...
  private:
    ...
    void set_thread(int i)
    {
            ...
            while (true)
            {
                while (isPop)
                {
                    ...
                    if (r) // can it be executed now?
                    {
                        ...
                        std::unique_ptr<executor_t> func(
                          fr.first); // at return, delete the function even if an exception occurred
        ...
        threads[i].reset(new thread_ns::thread(f)); // compiler may not support std::make_unique()
    }
