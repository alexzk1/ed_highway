...
EResult decompress(const uint8_t *src, std::size_t src_size, uint8_t *dst,
                   std::size_t init_dst_size, std::size_t &dst_size)
{
...
class DictImpl : public DictBase
{
  public:
    struct Match3Impl : DictBase::Match3
    {
        ...
        {
            return ((0x9f5f * (((uint32_t(data[0]) << 5 ^ uint32_t(data[1])) << 5) ^ data[2])) >> 5)
                   & 0x3fff;
        }
...
static void find_better_match(const uint32_t best_off[MaxMatchByLengthLen], uint32_t &lb_len,
                              uint32_t &lb_off)
{
...
static EResult encode_literal_run(uint8_t *&outp, const uint8_t *outp_end, const uint8_t *dst,
                                  std::size_t &dst_size, const uint8_t *lit_ptr, uint32_t lit_len)
{
...
static EResult encode_lookback_match(uint8_t *&outp, const uint8_t *outp_end, const uint8_t *dst,
                                     std::size_t &dst_size, uint32_t lb_len, uint32_t lb_off,
...
EResult compress(const uint8_t *src, std::size_t src_size, uint8_t *dst, std::size_t init_dst_size,
                 std::size_t &dst_size, DictBase &dict)
{
    ...
    while (s.buf_sz > 0)
    {
        ...
        if ((err = encode_literal_run(outp, outp_end, dst, dst_size, lit_ptr, lit_len))
            < EResult::Success)
            return err;
        if ((err = encode_lookback_match(outp, outp_end, dst, dst_size, lb_len, lb_off, lit_len))
            < EResult::Success)
            return err;
        lit_len = 0;
