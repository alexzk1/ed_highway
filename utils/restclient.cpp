...
{
    return dosomething(url, [](CURL * curl)->void
    {
        (void)curl;
        curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
    }, headers, timeout).setMethod("GET");
}
/**
...
{
    return dosomething(url, [&data, &func](CURL * curl)->void
    {
        //curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "POST");
        /** set post fields */
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, data.c_str());
        curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE_LARGE, data.size()); //allowing more then 2Gb by _LARGE
        func(curl);
    }, headers, timeout).setMethod("POST");
}

RestClient::response RestClient::patch(const std::string &url, const std::string &data, const size_t timeout)
{
    const static headermap emptyMap;
    return RestClient::patch(url, data, emptyMap, timeout);
}

RestClient::response RestClient::patch(const std::string &url, const std::string &data, const RestClient::headermap &headers, const size_t timeout, const RestClient::builder &func)
{
    return dosomething(url, [&data, &func](CURL * curl)->void
    {
        ...
        curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE_LARGE, data.size()); //allowing more then 2Gb by _LARGE
        func(curl);
    }, headers, timeout).setMethod("PATCH");
}

RestClient::response RestClient::del_put(const std::string &url, const std::string &data, const size_t timeout)
{
    const static headermap emptyMap;
...
RestClient::response RestClient::del_put(const std::string &url, const std::string &data, const RestClient::headermap &headers, const size_t timeout, const RestClient::builder &func)
{
    return dosomething(url, [&data, &func](CURL * curl)->void
    {
        ...
        curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE_LARGE, data.size()); //allowing more then 2Gb by _LARGE
        func(curl);
    }, headers, timeout).setMethod("DELETE");
}
...
RestClient::response RestClient::put(const std::string& url,
                                     const std::string& data,
                                     const headermap& headers,
                                     const size_t timeout, const builder &func)
{
    ...
    return dosomething(url, [&up_obj, &func](CURL * curl)->void
    {
...
RestClient::response RestClient::options(const std::string &url, const RestClient::headermap &headers, const size_t timeout)
{
    return custom_method_get_family("OPTIONS", url, headers, timeout);
}

RestClient::response RestClient::custom_method_get_family(const std::string &method, const std::string &url, const RestClient::headermap &headers, const size_t timeout, bool keepalive)
{
    return dosomething(url, [&method, keepalive](CURL * curl)->void
    {
    ...
    }, headers, timeout).setMethod(method);
}
...
RestClient::response RestClient::dosomething(const std::string &url, const RestClient::builder &func,
        const RestClient::headermap &headers, const size_t timeout)
{
    ...
    if (curl)
    {
        ...
        curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, std::min(static_cast<size_t>(20), timeout + 1));
        /** set user agent */
