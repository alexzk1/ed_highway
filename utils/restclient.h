...
class RestClient
{
  public:
    ...
    static response post(
      const std::string &url, const std::string &data, const headermap &headers,
    ...
    static response patch(const std::string &url, const std::string &data,
    ...
    static response del_put(const std::string &url, const std::string &data,
                            const size_t timeout = 0);
    static response del_put(
      const std::string &url, const std::string &data, const headermap &headers,
      const size_t timeout = 0, const builder &func = [](CURL *curl) {
          (void)curl;
      });
    ...
    // HTTP PUT
    static response put(const std::string &url, const std::string &data, const size_t timeout = 0);
    static response put(
      const std::string &url, const std::string &data, const headermap &headers,
      const size_t timeout = 0, const builder &func = [](CURL *curl) {
          (void)curl;
    ...
    static response dosomething(const std::string &url, const builder &func,
                                const headermap &headers, const size_t timeout = 0);

    // HTTP DELETE
    static response del(const std::string &url, const size_t timeout = 0);
    static response del(const std::string &url, const headermap &headers, const size_t timeout = 0);

    // options
    static response options(const std::string &url, const headermap &headers,
                            const size_t timeout = 0);

    static response custom_method_get_family(const std::string &method, const std::string &url,
                                             const headermap &headers, const size_t timeout = 0,
