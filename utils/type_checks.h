...
namespace types_ns
{
    ...
    template <typename... Args> struct ismap<std::multimap<Args...>>: std::true_type {};

    template <typename T>       struct ispair: std::false_type {};
    template <typename... Args> struct ispair<std::pair<Args...>>: std::true_type {};

    template <typename T>            struct isclassarray: std::false_type {};
    template<class T, std::size_t N> struct isclassarray<std::array<T, N>>: std::true_type {};
...
struct is_callable < T(Args......) &&, U > : std::true_type {};
    ...
    struct ArrayTest
    {
        static_assert(types_ns::isarray<T>::value || types_ns::isclassarray<T>::value || types_ns::isvector<T>::value, "T must be an array or vector type.");
