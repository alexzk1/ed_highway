...
{
    // FIXME: implement radio-group too, so can be selected single option (and all needed to switch
    // modes)
    (void)multi_select;
    ...
    addContainerAsCheckboxes(
      containerLayout, edsmStarClasses(), [&](const QPointer<QCheckBox> &checkbox, auto) {
...
{
    // this should return true if star class (1 or couple) is selected, which means filter is in
    // effect
